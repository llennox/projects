...
class RegistrationForm(forms.Form):
    ...
    def sendEmail(self, datas):
        link="http://http://localhost:8000/activate/"+datas['activation_key']
        ...
        send_mail(datas['email_subject'], message, 'gonnellcough@gmail.com', [datas['email']], fail_silently=False)
...
class LeaveCommentForm(forms.Form):
     message = forms.CharField(required=True,label="",widget=forms.Textarea(attrs={'rows':'5','columns':'51','id':'commentInput','message':'message_input'}),max_length=255,min_length=3)#,validators=[isValidUsername, validators.validate_slug])

class UploadForm(forms.Form):
    caption = forms.CharField(max_length=151, required=False)
    photo = forms.ImageField(widget=forms.ClearableFileInput(attrs={'style' : 'margin-left: auto; margin-right: auto; width: auto; float: center;text-align: center'}))
...
class ProfileBioForm(forms.Form):
    bio = forms.CharField(max_length=151, required=True)
